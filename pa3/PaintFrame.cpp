...
void PaintFrame::OnExport(wxCommandEvent& event)
{
    ...
    new wxFileDialog(this, _("Save File As"), "", "", _("PNG File (*.png)|*.png|BMP File (*.bmp)|*.bmp|JPEG File (*.jpeg, *jpg)|*.jpeg;*.jpg"),
          wxFD_SAVE | wxFD_OVERWRITE_PROMPT, wxDefaultPosition);
        
        // Creates a Save Dialog with 3 file types
        if (SaveDialog->ShowModal() == wxID_OK) // If the user clicked "OK"
        {
            mModel->ExportFile(SaveDialog->GetPath(), mPanel->GetSize());
        }
...
void PaintFrame::OnImport(wxCommandEvent& event)
{
    ...
    new wxFileDialog( this, _("Choose a file to open"), "", "", _("PNG Files (*.png)|*.png|BMP Files (*.bmp)|*.bmp|JPEG Files (*.jpeg, *jpg)|*.jpeg;*.jpg"),
                     wxFD_OPEN, wxDefaultPosition);
    
    if ( openDialog->ShowModal() == wxID_OK )
    {
        mModel->LoadImport(openDialog->GetPath());
        mPanel->PaintNow();
    }
    // Clean up after ourselves
    openDialog->Destroy();
...
void PaintFrame::OnDelete(wxCommandEvent& event)
{
    ...
    {
        mEditMenu->Enable(ID_Unselect, false);
        wxPoint startpoint = mModel->GetSelectedShape()->getStartPoint();
        mModel->CreateCommand(CM_Delete, startpoint);
...
void PaintFrame::OnSetPenColor(wxCommandEvent& event)
{
    ...
    if (dialog.ShowModal() == wxID_OK)
    {
        ...
        if (mModel->HasSelectedShape())
        {
            ...
            mModel->CreateCommand(CM_SetPen, startpoint);
            mModel->FinalizeCommand();
        }
        else
        {
...
void PaintFrame::OnSetPenWidth(wxCommandEvent& event)
{
	// TODO
    wxString valueTyped;
    wxTextEntryDialog myDialog(this, _("Enter an integer value from 1-10"), _("Pen Width"), _(""));
    if ( myDialog.ShowModal() == wxID_OK )
    {
        valueTyped = myDialog.GetValue();
        if (wxAtoi(valueTyped) >= 1 && wxAtoi(valueTyped) <= 10)
        {
            mModel->SetPenWidth(wxAtoi(valueTyped));
        }
    }
}
...
void PaintFrame::OnSetBrushColor(wxCommandEvent& event)
{
    ...
    if (dialog.ShowModal() == wxID_OK)
    {
        ...
        if (mModel->HasSelectedShape())
        {
            ...
            mModel->CreateCommand(CM_SetBrush, startpoint);
            mModel->FinalizeCommand();
        }
        else
        {
...
void PaintFrame::OnMouseButton(wxMouseEvent& event)
{
	if (event.LeftDown())
	{
        ...
        if (mCurrentTool == ID_DrawRect)
        {
            ...
            mModel->CreateCommand(CM_DrawRect, point);
            mPanel->PaintNow();
        }
        else if (mCurrentTool == ID_DrawEllipse)
        {
            wxPoint point = event.GetPosition();
            mModel->CreateCommand(CM_DrawEllipse, point);
            mPanel->PaintNow();
        }
        else if (mCurrentTool == ID_DrawLine)
        {
            wxPoint point = event.GetPosition();
            mModel->CreateCommand(CM_DrawLine, point);
            mPanel->PaintNow();
        }
        else if (mCurrentTool == ID_DrawPencil)
        {
            wxPoint point = event.GetPosition();
            mModel->CreateCommand(CM_DrawPencil, point);
            mPanel->PaintNow();
        }
	}
